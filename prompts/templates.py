...
Each step has an execution_mode field: LLM_PROMPT | PROMPT_TO_CODE | CODE | USER_INPUT | NETWORK.
...
## C. Alternative flows (for [ALTERNATIVE_FLOW: condition] blocks)
...
## D. Exception flows (for [EXCEPTION_FLOW: condition] LOG blocks)
...
def render_s4c_user(entities_text: str, omit_files_text: str) -> str:
    return (S4C_USER
            .replace("{{entities_text}}", entities_text)
            .replace("{{omit_files_text}}", omit_files_text))
